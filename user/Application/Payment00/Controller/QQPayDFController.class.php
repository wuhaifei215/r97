...
class QQPayDFController extends PaymentController
{
    private $code = '';
    ...
    public function PaymentExec($data, $config)
    {
        $post_data = array(
            ...
            "order_num" => $data['orderid'], //订单号
            "currency" => "BRL",
            "user_info_type" => "CPF",
            "account_no" => $data['banknumber'],    //账号
            "account_name" => $data['bankfullname'],  //户名
            'bank_number' => $data['type'],
            "order_fee" => sprintf("%.2f", $data['money']) * 100,  //提现金额
        ...
        // if($data['userid'] == 2){
            try{
                ...
                logApiAddPayment('YunPay订单提交三方QQPay', __METHOD__, $data['orderid'], $data['out_trade_no'], $config['exec_gateway'], $post_data, $result, $doTime, '0', '1', '2');
            }catch (\Exception $e) {
                // var_dump($e);
            }
        // }
    ...
    public function notifyurl()
    {
        ...
        if ($sign === $re_data["sign"]) {
            if ($re_data['status'] === "2") {      //⽀付状态, -2-订单过期,-1-⽀付失败,0-订单⽣成,1-⽀付中,2-⽀付成功,3-业务处理完成
                $data = [
                    'memo' => '代付成功',
                ];
            ...
            } elseif ($re_data['status'] === "-2" || $re_data['status'] === "-1") {
                //代付失败
                $data = [
    ...
    public function PaymentQuery($data, $config)
    {
        ...
        $result = json_decode($returnContent, true);
        if ($result['code'] === 200) {
            switch ($result['data']['status']) {       //⽀付状态, -2-订单过期,-1-⽀付失败,0-订单⽣成,1-⽀付中,2-⽀付成功,3-业务处理完成
                case 0:
                case 1:
                ...
                case 2:
                    ...
                    break;
                case 3:
                    $return = ['status' => 3, 'msg' => $result['msg'],'remark' => $result['msg']];
                    break;
            }
        } else {
    ...
    public function queryBalance()
    {
        if (IS_AJAX) {
            ...
            $result = json_decode($returnContent, true);
            if($result['code'] === 200){
                $balance = $result['data']['money'] / 100;  //总金额
                $html = <<<AAA
...
AAA;
    ...
    public function queryBalance2($config)
    {
        ...
        if($result['status'] === 1){
            ...
            $result_data['balance'] = $result['data']['money'] / 100;
        }
        return $result_data;
    }
    //发送post请求，提交data字符串
