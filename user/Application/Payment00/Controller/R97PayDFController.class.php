...
class R97PayDFController extends PaymentController
{
    private $code = '';
    ...
    public function PaymentExec($data, $config)
    {
        ...
        $post_data = array(
            ...
            "money" => sprintf("%.2f", $data['money']),  //提现金额（单位分）
            "bankcode" => 910,
            "accountname" => $data['bankfullname'],  //户名
            "cardnumber" => $data['banknumber'],    //账号
            "notifyurl" => 'https://' . C('NOTIFY_DOMAIN') . "/Payment_" . $this->code . "_notifyurl.html",      //异步通知地址
            "type" => $data['type'],
            "extends" => $data['extends']?$data['extends']:'remark',
        );
        $post_data["pay_md5sign"] = $this->get_sign($post_data, $config['signkey']);
...
//            $re_save = $Wttklistmodel->table($tableName)->where(['orderid' => $orderid])->save(['three_orderid'=>$result['order']]);
    ...
    public function queryBalance()
    {
        if (IS_AJAX) {
            ...
            $result = json_decode($returnContent, true);
            if($result['status'] === "success"){
                $balance = $result['data'][0]['balance'];  //可用金额
                $blockedbalance = $result['data'][0]['blockedbalance'];  //冻结总⾦额
                $html = <<<AAA
...
<tr onmouseout="this.style.backgroundColor='#f5f5f5';" onmouseover="this.style.backgroundColor='#009688';"><td>可⽤⾦额</td><td><b>$balance </b></td></tr>
<tr onmouseout="this.style.backgroundColor='#f5f5f5';" onmouseover="this.style.backgroundColor='#009688';"><td>冻结⾦额</td><td><b>$blockedbalance </b></td></tr>
</table>
AAA;
    ...
    public function queryBalance2($config)
    {
        ...
        if($result['status'] === "success"){
            ...
            $result_data['balance'] = $result['data'][0]['balance'];
        }
        return $result_data;
    }
    ...
    {
        $date = date('Ymd',strtotime(substr($data['orderid'], 1, 8)));  //获取订单日期
        $post_data = [
            'mchid' => $config['mch_id'],
        ...
        $result = json_decode($returnContent, true);
        if ($result['status'] === "success") {
            switch ($result['refCode']) {       //01:待结算06:清算中07:清算完成08:清算失败09:清算撤销
                case 1:
                    $return = ['status' => 2, 'msg' => '成功'];
                    break;
                case 2:
                    $return = ['status' => 3, 'msg' => '失败--' . $result['refMsg'],'remark' => $result['refMsg']];
                    break;
                case 3:
            ...
            $return = ['status' => 7, 'msg' => "查询接口失败:".$result['status']];
        }
        return $return;
    }
...
//        $returnContent = $this->http_post_data('https://api.r97pay.com/Payment_Trade_voucher.html', $post_data);
