...
class ReceiptLogModel extends Model {
    protected $expire_date = 20241230;
    protected $tablePrefix = 'custom_'; // 分表前缀
    protected $tableName = 'receipt_log'; // 默认表名
    protected $orderTables=[];      //订单分表表名
    protected $timeoptions = ['create_time'];
    // 数据库表达式
    ...
    protected function createTable(){
        ...
        if(!$isTable){
            ...
            $creatSql = "CREATE TABLE `" . $tableName . "` (
                  ...
                  `out_trade_id` varchar(36) DEFAULT NULL COMMENT '下游订单号',
                  `business_type` int(11) DEFAULT '0' COMMENT '业务类型（0其它 1下单 2回调）',
                  `method` varchar(200) DEFAULT '' COMMENT '方法名称',
                  `request_method` varchar(10) DEFAULT '' COMMENT '请求方式',
                  `operator_type` int(11) DEFAULT '0' COMMENT '操作类别（0其它 1后台 2Api）',
                ...
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='入款日志'";
            $this->execute($creatSql);
        }
    }
        
    public function getTables($data=array()){
        if(!empty($data) && $data['create_time']){
            //获取时间范围内的表
    ...
    public function setRealTableName($date =array()) {
        $date = isset($data['create_time']) ? $data['create_time'] : date('Ymd',time());
        $realTableName = $this->getRealTableName($date);
        // 设置当前模型对应的数据表
    ...
    public function saveByDate($data, $options = array()){
        // 根据创建时间计算应该使用的分表
        $date = isset($options['create_time']) ? $options['create_time'] : date('Ymd',time());
        $tableName = $this->getRealTableName($date);
        // 切换到对应的分表进行更新操作
    ...
    public function new_parseDate($options){
        ...
        if(isset($options['create_time'])){
            if(is_array($options['create_time'])){
                $startdate = $options['create_time'][1][0];
                $enddate = $options['create_time'][1][1];
            }else{
                $date = $options['create_time']?$options['create_time']:date('Y-m-d',time());
