...
use yy\exception\InvalidResponseException;
use yy\sign\RSAUtil;
...
class HttpCurlUtil
{
    ...
    private function setHeader(){
        ...
        $defaultHeader = array(
            'lang:PHP',
            'publisher:yy',
            'sdk-version:'.PayConfig::$SDK_VERSION,
            'uname:'.php_uname(),
     ...
     * @return unknown
     */
    public function get($isVerifySign=true,$url = '', $vars = array(), $timeout = 5){
        $this->setUrl($url);
        $this->setHeader();
        $this->setVar($vars);
        return $this->send($isVerifySign,'GET', $timeout);
    }
     ...
     * @return unknown
     */
    public function post($isVerifySign=true,$url = '', $vars = array(), $timeout = 5 ){
        $this->setUrl($url);
        $this->setHeader();
        $this->setVar($vars);
        return $this->send($isVerifySign,'POST', $timeout);
    }
     ...
     * @throws Exception
     */
    private function send($isVerifySign=true,$method = 'GET', $timeout = 5, $options = array()){
        //处理参数是否为空
        if ($this->uri == ''){
        ...
        $_json_data = json_encode($this->vars,JSON_UNESCAPED_UNICODE);
