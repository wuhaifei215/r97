...
class WopPayController extends PayController
{
    private $code = '';
    ...
    public function Pay($array)
    {
        ...
        if($ans['code'] === 0){
            $payurl = $site . 'PayPage.html?sid=' . $return['orderid'] . '&amount=' . $return['amount']. '&qrcode=' .$ans['data']['payData'];
            $return_arr = [
                'status' => 'success',
                'H5_url' => $payurl,
                'QRcode' => $ans['data']['payData'],
                'pay_orderid' => $orderid,
        ...
        // if($array['userid'] == 2){
            try{
                ...
                logApiAddReceipt('YunPay订单提交上游WopPay', __METHOD__, $return['orderid'], $return['out_trade_id'], $return['gateway'], $native, $ans, $doTime, '0', '1', '2');
            }catch (\Exception $e) {
                // var_dump($e);
            }
        // }
    ...
    public function notifyurl()
    {
        ...
        if ($sign == $result["sign"]) {
            if($result['payStatus'] === "1"){     //订单状态 0:未支付；1：支付成功
                ...
                log_place_order($this->code . '_notifyurl', $orderid . "----订单状态异常", $result['payStatus']);    //日志
            }
        } else {
