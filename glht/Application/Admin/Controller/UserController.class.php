...
//                            } else {
                                if ($item['feilv'] < $channel['rate']) {
                                    $this->ajaxReturn(['status' => 0, 'msg' => '【' . $channel['title'] . '】 运营费率不得低于渠道的成本费率：' . ($channel['rate'] * 100) . '%']);
                                }
                                if ($item['sxffixed'] < $channel['sxffixed']) {
                                    $this->ajaxReturn(['status' => 0, 'msg' => '【' . $channel['title'] . '】 运营单笔不得低于渠道的成本单笔：' . $channel['sxffixed']]);
                                }
//                            }
//                        }
                    }
    ...
    public function saveUserChannel()
    {
        ...
        if (IS_POST) {
            ...
            foreach ($u as $key => $item) {
                ...
                if ($product) {
                    $data = $data_insert[] = ['id' => $product['id'], 'userid' => $userid, 'pid' => $key, 'status' => $status, 'channel' => $item['channel']];
                } else {
                    $data = $data_update[] = ['userid' => $userid, 'status' => $status, 'pid' => $key, 'channel' => $item['channel']];
                }
            }
            ...
            $redis = redis_connect();
            foreach ($data as $dv) {
                $userPayForAnother = M('UserPayForAnother')->where(['pid' => $dv['pid'], 'userid' => $dv['userid']])->find();
                $redis->set('UserPayForAnother_'. $dv['pid'] . '_' . $dv['userid'], json_encode($userPayForAnother, JSON_UNESCAPED_UNICODE));
                $redis->expire('UserPayForAnother_'. $dv['pid'] . '_' . $dv['userid'], 60);
            }
            $this->ajaxReturn(['status' => 1]);
        }
    }
        ...
        if (IS_POST) {
            ...
            if(!empty($ids_arr)){
                ...
                foreach ($ids_arr as $v){
                    ...
                    $productUser = M('ProductUser')->where(['pid' => $product, 'userid' => $v])->find();
                    $redis->set('ProductUser_'. $product . '_' . $v, json_encode($productUser, JSON_UNESCAPED_UNICODE));
                    $redis->expire('ProductUser_'. $product . '_' . $v, 60);
                }
                $this->ajaxReturn(['status' => 1]);
            }
            $this->ajaxReturn(['status' => 0]);
        ...
        if (IS_POST) {
            ...
            if(!empty($ids_arr)){
                ...
                foreach ($ids_arr as $v){
                    ...
                    $userPayForAnother = M('UserPayForAnother')->where(['pid' => $product, 'userid' => $v])->find();
                    $redis->set('UserPayForAnother_'. $product . '_' . $v, json_encode($userPayForAnother, JSON_UNESCAPED_UNICODE));
                    $redis->expire('UserPayForAnother_'. $product . '_' . $v, 60);
                }
                $this->ajaxReturn(['status' => 1]);
            }
            $this->ajaxReturn(['status' => 0]);
