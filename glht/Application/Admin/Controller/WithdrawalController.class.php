...
class WithdrawalController extends BaseController
{
    ...
    public function saveURate(){
        if (IS_POST) {
            ...
            if($rate>0){
                ...
                if($res){
                    UserLogService::HTwrite(5, '保存U汇率', '保存U汇率：' . $rate . ' ,成功');
                    $this->ajaxReturn(['status' => 1,'msg' => '设置U汇率成功!']);
                }
            }
            UserLogService::HTwrite(5, '保存U汇率',  '保存U汇率：' . $rate . ' ,失败');
    ...
    public function submitDf()
    {
        ...
        if (IS_POST) {
            ...
            $where['status'] = 1;
            if($channe_code){
                $where['id'] = $channe_code;
            }else{
                $where['is_default'] = 1;
            }
            $channe = M('PayForAnother')->where($where)->find();
            ...
            $this->assign('channe_title', $channe['title']);
            $this->assign('verifysms', $verifysms);
            $this->assign('verifyGoogle', $verifyGoogle);
            $this->assign('auth_type', $verifyGoogle ? 1 : 0);
            $this->display();
                            ...
                            if (is_array($result)) {
                                ...
                                switch($result['status']){
                                    ...
                                    default:
                                        ...
                                        $change_data['memo']  = 'Status does not change！ - '. $result['msg'];
                                        break;
                                }
